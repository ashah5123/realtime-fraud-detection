...
@pytest.fixture
def ensemble():
    """EnsembleScorer with default thresholds (low=0.3, high=0.7) and three-way weights."""
    e = EnsembleScorer()
...
def test_score_in_range(ensemble):
    ...
    result = ensemble.score_transaction(0.0, 0.0, 0.0)
    assert 0 <= result["fraud_score"] <= 1
    result = ensemble.score_transaction(1.0, 1.0, 1.0)
    assert 0 <= result["fraud_score"] <= 1
    result = ensemble.score_transaction(0.5, 0.5, 0.5)
    assert 0 <= result["fraud_score"] <= 1
...
def test_risk_tier_low(ensemble):
    ...
    result = ensemble.score_transaction(0.1, 0.1, 0.1)
...
def test_risk_tier_medium(ensemble):
    ...
    result = ensemble.score_transaction(0.4, 0.4, 0.4)
...
def test_risk_tier_high(ensemble):
    ...
    result = ensemble.score_transaction(0.9, 0.9, 0.9)
    assert result["risk_tier"] == "HIGH"
    result = ensemble.score_transaction(1.0, 0.8, 0.8)
    assert result["risk_tier"] == "HIGH"
